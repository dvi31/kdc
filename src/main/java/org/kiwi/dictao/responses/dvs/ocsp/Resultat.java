...
public class Resultat extends GeneralResultat<OCSPResp> {
    ...
    public boolean isValid() {
        try {
            ...
            //Signature valide (vaFile) 
            if (!basicResponse.verify(signKey, "BC")) {
                return false;
            }
            ...
            //Signature (caFile, OCSP), throw exception if invalid
            signOCSP.verify(emmeteurSignature.getPublicKey(), "BC");
            //Signature valide (OCSP)
            if (!basicResponse.verify(signOCSP.getPublicKey(), "BC")) {
                return false;
            }
